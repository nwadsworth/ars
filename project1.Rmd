...
ars <- function(formula, nsamples, min = -Inf, max = Inf, xinit) {
  ...
  while(i < nsamples){
    find_x_star(cdf, int_u, d, z, t, h, u)
...
Initialization step
```{r}
make_u <- function(i) { force(i); function(x) (data[i, 2] + (x - data[i, 1]) * data[i, 3]) }
...
make_l <- function(i) { 
  force(i)
  function(x) ((data[i + 1, 1] - x) * data[i, 2] + (x - data[i, 1]) * data[i + 1, 2]) / (data[i + 1, 1] - data[i, 1])
}
