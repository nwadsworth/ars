...
# d <-check_dist(formulas=f, nsamples=100, min=0, max=3, xinit=c(-0.5,0.2, 0.8, 1.3))
# d
# is.numeric(d)
###############################################################################
ars <- function(formulas=dnorm, nsamples=100, min = -Inf, max = Inf, xinit=c(-1.5,-0.2, 0.5, 1)) {
  xinit=sort(unique(xinit))
  xinit=xinit[(xinit>min)&(xinit<max)]
  res=check_dist(formulas=formulas, nsamples=nsamples, min=min, max=max, xinit=xinit)
  if(is.numeric(res)){
    return(res)
  }
  if(res==FALSE){
...
system.time({
  res=ars(formulas=g, nsamples=100000, min = 1, max = 4, xinit=c(-1.4,0.2, 0.5, 3))
})
